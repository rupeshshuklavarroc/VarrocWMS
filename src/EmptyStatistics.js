...
import EmptyStatusDisplay from "./EmptyStatusDisplay";
    ...
    className={`drawer-container ${state[anchor] ? "drawer-open" : ""}`}>
      ...
      <List className="drawer-list" >
        {['Entry','Issue','Analytics','Live Status','Transaction Log','Log Out'].map((text,index)=>(
          <ListItem className="drawer-item"  key={text} disablePadding>
              <ListItemIcon>
            ...
            <ListItemButton className="list-item-button"  onClick={()=>{
              if(text==="Entry"){
                handleEntryClick();
                console.log(text)
              }else if(text==="Issue"){
...
export default function EmptyStatisticsWithGlobalProvider() {
  return (
    <GlobalProvider>
