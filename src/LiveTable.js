...
function LiveTable() {
  ...
  const list = (anchor) => (
    ...
    className={`drawer-container ${state[anchor] ? "drawer-open" : ""}`}>
      ...
      <List className="drawer-list" >
        {['Entry','Issue','Analytics','Transaction Log','Stock Analysis','Log Out'].map((text,index)=>(
  ...
  return (
    <div className="App">
      ...
      <div>
        {/* Render the LiveDataDisplay component */}
        <LiveDataDisplay />
      </div>
    </div>
  );
...
export default function LiveWithGlobalProvider() {
  return (
    <GlobalProvider>
      {/* Render the LiveTable component wrapped with GlobalProvider */}
      <LiveTable />
    </GlobalProvider>
  );
}
